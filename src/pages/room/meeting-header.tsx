...
const MeetingHeader: React.FC<{
  setLayour: React.Dispatch<React.SetStateAction<'grid' | 'list'>>;
}> = ({ setLayour }) => {
  ...
  return (
    <header className="flex content-stretch items-center px-2 py-2 text-xs justify-between">
      ...
      <button
        className="btn btn-text flex items-center gap-1 px-1 py-0 font-normal "
        onClick={() => setLayour((d) => (d === 'grid' ? 'list' : 'grid'))}
      >
